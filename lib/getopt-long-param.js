...
getoptLongParam.prototype.arrayTest = function(count, value) {
    for ( var i = 0; i < this.test[count].length; i++ ) {
        if (this.test[count][i] === value) {
            return value;
        }
    }

    throw '--' + this.name + ' must be one of ' + this.test[count].join(', ') + '\n';
};
