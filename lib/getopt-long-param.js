...
getoptLongParam.prototype.process = function(arg) {
    ...
    var auto  = this.autoNo ? arg.match(/^-(\d+)$/) : false;
    ...
    if ( long || short || auto ) {
        ...
        if (this.increment) {
            ...
            var valCount = this.extractValue(key, value, short, auto, arg, arguments);
            value = valCount[0];
...
getoptLongParam.prototype.extractValue = function(key, value, short, auto, arg, processArg) {
    if (auto) {
        return [ auto[1] ];
    }
    else if (arg.match(/=/)) {
        return [ strSplit(arg, '=', 2)[1] ];
    }
    else if (short && arg.length > 2) {
        return [ arg.substr(2) ];
    }
    else if (processArg.length > 1) {
        return [ processArg[1], 2 ];
    }
